...
export async function POST() {
  try {
    ...
    const alterStatements = [
      ...
      `ALTER TABLE skills ADD COLUMN IF NOT EXISTS has_embedding BOOLEAN DEFAULT FALSE`,
      `ALTER TABLE skill_evolution ADD COLUMN IF NOT EXISTS change_type TEXT`,
      `ALTER TABLE skill_evolution ADD COLUMN IF NOT EXISTS previous_state JSONB`,
      `ALTER TABLE skill_evolution ADD COLUMN IF NOT EXISTS new_state JSONB`,
      `ALTER TABLE skill_evolution ADD COLUMN IF NOT EXISTS change_reason TEXT`,
      `ALTER TABLE skill_evolution ADD COLUMN IF NOT EXISTS trigger_agent_id TEXT`,
      `ALTER TABLE skill_executions ADD COLUMN IF NOT EXISTS quality_score NUMERIC`,
      `ALTER TABLE skill_executions ADD COLUMN IF NOT EXISTS steps_taken INTEGER`,
      `ALTER TABLE skill_executions ADD COLUMN IF NOT EXISTS tokens_used INTEGER`,
      `ALTER TABLE skill_executions ADD COLUMN IF NOT EXISTS feedback TEXT`,
      `ALTER TABLE skill_executions ADD COLUMN IF NOT EXISTS error_message TEXT`,
      `ALTER TABLE skill_executions ADD COLUMN IF NOT EXISTS metadata JSONB`,
      `ALTER TABLE skill_executions ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ`,
      `CREATE TABLE IF NOT EXISTS skill_evaluations (
        id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
        skill_id UUID NOT NULL,
        agent_id TEXT,
        task_id TEXT,
        evaluation_type TEXT DEFAULT 'automated',
