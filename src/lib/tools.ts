...
export const createPdfReportTool = tool({
  description: "Create a professional PDF report and return it as a downloadable file. Produces industry-standard documents with cover page, table of contents, page numbers, headers/footers, and polished formatting. Supports markdown: headers, lists, tables, bold, italic, code blocks. Use this when the user asks to generate a PDF, report, whitepaper, proposal, or any PDF file.",
  inputSchema: zodSchema(z.object({
  ...
  execute: safeJson(async ({ title, content, filename, author, subtitle }) => {
    ...
    const C = {
      ...
      tableBg: "#EFF6FF",      // Table header bg
      tableAlt: "#F8FAFC",     // Table alt row
      codeBg: "#F1F5F9",       // Code block bg
      codeBorder: "#CBD5E1",   // Code block border
      quoteBorder: "#3B82F6",  // Blockquote border
      quoteBg: "#EFF6FF",      // Blockquote bg
      white: "#FFFFFF",
    };
    ...
    const registerToc = (level: number, ttl: string) => {
      // Store placeholder — page number will be updated after all pages are rendered
      tocEntries.push({ level, title: ttl, page: doc.bufferedPageRange().start + 1 });
    };
    ...
    const addFooter = () => {
      ...
      // Footer line
      doc.moveTo(ML, PH - 52).lineTo(PW - MR, PH - 52).strokeColor(C.border).lineWidth(0.5).stroke();
      // Page number — right aligned
      doc.text(`Page ${pageNum}`, ML, PH - 48, { width: CW, align: "right" });
      // Footer text — left aligned
      doc.text(`${title}`, ML, PH - 48, { width: CW * 0.6, align: "left" });
      doc.restore();
    };
    ...
    // Title
    doc.fontSize(28).font("Helvetica-Bold").fillColor(C.primary).text(title, ML, doc.y, {
    ...
    // Subtitle
    if (subtitle) {
      doc.fontSize(14).font("Helvetica").fillColor(C.secondary).text(subtitle, ML, doc.y, {
    ...
    // Decorative divider
    const divY = doc.y + 8;
    doc.moveTo(PW / 2 - 60, divY).lineTo(PW / 2 + 60, divY).strokeColor(C.secondary).lineWidth(2).stroke();
    doc.moveTo(PW / 2 - 40, divY + 6).lineTo(PW / 2 + 40, divY + 6).strokeColor(C.accent).lineWidth(0.5).stroke();
    ...
    doc.fontSize(20).font("Helvetica-Bold").fillColor(C.primary).text("Table of Contents", ML, MT);
    doc.moveDown(0.5);
    doc.moveTo(ML, doc.y).lineTo(PW - MR, doc.y).strokeColor(C.secondary).lineWidth(1).stroke();
    doc.moveDown(1);
    ...
    // Render TOC entries (page numbers will be placeholders "..." — updated below)
    for (const h of headingList.slice(0, 50)) {
      if (doc.y > PH - 100) doc.addPage();
      const indent = h.level === 1 ? 0 : h.level === 2 ? 18 : 36;
      const fontSize = h.level === 1 ? 11 : h.level === 2 ? 10 : 9;
      const fontName = h.level === 1 ? "Helvetica-Bold" : "Helvetica";
      const textColor = h.level === 1 ? C.primary : C.text;

      doc.fontSize(fontSize).font(fontName).fillColor(textColor)
        .text(h.title, ML + indent, doc.y, { width: CW - indent - 40, continued: false });
      // Dots and page placeholder — we'll come back to fix page numbers
      doc.moveDown(0.2);
    }
    ...
    let tocIdx = 0; // Track which heading we're at for TOC page numbers
    ...
    for (const line of lines) {
      ...
      if (line.trim().startsWith("```")) {
        if (inCodeBlock) {
          // End code block — draw closing border
          const codeEndY = doc.y + 4;
          doc.rect(ML + 8, codeStartY!, CW - 16, codeEndY - codeStartY!)
            .lineWidth(0.5).strokeColor(C.codeBorder).stroke();
          inCodeBlock = false;
          doc.y = codeEndY + 8;
        } else {
          // Start code block
          ...
          codeStartY = doc.y; // Record start position for background rect
          doc.rect(ML + 8, codeStartY, CW - 16, 4).fill(C.codeBg); // min height placeholder
          doc.moveDown(0.4);
          // Show language label
          if (codeLang) {
            doc.fontSize(7).font("Helvetica-Bold").fillColor(C.accent)
              .text(codeLang.toUpperCase(), ML + 14, doc.y, { width: CW - 32 });
            doc.moveDown(0.2);
          }
        }
        continue;
      ...
      if (inCodeBlock) {
        doc.fontSize(8).font("Courier").fillColor("#334155")
          .text(line || " ", ML + 16, doc.y, { width: CW - 40, lineGap: 1.5 });
        continue;
      }
      ...
      if (line.trim().startsWith("> ")) {
        ...
        // End of blockquote — render accumulated lines
        inBlockquote = false;
        if (blockquoteLines.length > 0) {
          const bqText = blockquoteLines.join(" ");
          const bqY = doc.y;
          // Measure text height
          const textHeight = doc.fontSize(10).font("Helvetica-Oblique").heightOfString(bqText, { width: CW - 50 });
          doc.rect(ML + 8, bqY - 2, CW - 16, textHeight + 10).fill(C.quoteBg);
          doc.rect(ML + 8, bqY - 2, 3, textHeight + 10).fill(C.quoteBorder);
          doc.fontSize(10).font("Helvetica-Oblique").fillColor(C.primary)
            .text(bqText, ML + 22, bqY + 3, { width: CW - 50, lineGap: 2 });
          doc.moveDown(0.5);
        }
        blockquoteLines = [];
        continue;
      ...
      if (line.trim().startsWith("|")) {
        ...
        if (tableRows.length > 0) {
          ...
          for (let c = 0; c < colCount; c++) {
            const cellText = (tableRows[0][c] || "").slice(0, 30);
            doc.text(cellText, ML + c * colWidth + 6, doc.y - 18, {
              width: colWidth - 12, height: 20,
            });
          }
          doc.y += 6;

          // Data rows
          for (let r = 1; r < Math.min(tableRows.length, 100); r++) {
            if (doc.y > PH - MB - 40) {
              addFooter();
              doc.addPage();
              addHeader();
              ...
              doc.fontSize(9).font("Helvetica-Bold").fillColor(C.white);
              for (let c = 0; c < colCount; c++) {
                doc.text((tableRows[0][c] || "").slice(0, 30), ML + c * colWidth + 6, doc.y - 18, { width: colWidth - 12 });
              }
              doc.y += 6;
            }
            ...
            // Measure row height
            let maxH = 20;
            for (let c = 0; c < colCount && c < (tableRows[r]?.length || 0); c++) {
              const h = doc.fontSize(8).font("Helvetica").heightOfString((tableRows[r][c] || "").slice(0, 40), { width: colWidth - 12 });
              maxH = Math.max(maxH, h + 8);
            }

            doc.rect(ML, doc.y, CW, maxH).fill(rowBg);
            doc.moveTo(ML, doc.y + maxH).lineTo(ML + CW, doc.y + maxH).strokeColor(C.border).lineWidth(0.3).stroke();

            doc.fontSize(8).font("Helvetica").fillColor(C.text);
            for (let c = 0; c < colCount && c < (tableRows[r]?.length || 0); c++) {
              doc.text((tableRows[r][c] || "").slice(0, 40), ML + c * colWidth + 6, doc.y + 4, {
                width: colWidth - 12,
              });
            }
            // Vertical grid lines
            for (let c = 1; c < colCount; c++) {
              doc.moveTo(ML + c * colWidth, doc.y).lineTo(ML + c * colWidth, doc.y + maxH)
      ...
      if (h1Match) {
        // Register in TOC
        if (tocIdx < headingList.length) {
          registerToc(1, h1Match[1]);
          tocIdx++;
        }
        doc.moveDown(1);
        // Blue accent bar
      ...
      if (h2Match) {
        if (tocIdx < headingList.length) {
          registerToc(2, h2Match[1]);
          tocIdx++;
        }
        doc.moveDown(0.7);
        doc.fontSize(14).font("Helvetica-Bold").fillColor(C.secondary)
          .text(h2Match[1]);
        doc.moveTo(ML, doc.y + 2).lineTo(ML + CW * 0.3, doc.y + 2).strokeColor(C.accent).lineWidth(0.8).stroke();
        doc.moveDown(0.4);
        continue;
      }
      if (h3Match) {
        if (tocIdx < headingList.length) {
          registerToc(3, h3Match[1]);
          tocIdx++;
        }
        doc.moveDown(0.5);
      ...
      if (line.trim() === "---" || line.trim() === "***") {
        doc.moveDown(0.4);
        doc.moveTo(ML, doc.y).lineTo(ML + CW, doc.y).strokeColor(C.border).lineWidth(0.5).stroke();
        doc.moveDown(0.4);
        continue;
      }
      ...
      if (bulletMatch) {
        const indent = Math.min((bulletMatch[1].length / 2), 3); // 0-3 nesting levels
        const bulletX = ML + 16 + indent * 16;
        doc.fontSize(10).font("Helvetica").fillColor(C.text)
          .text("\u2022", bulletX, doc.y, { continued: true, width: 12 })
          .text(` ${bulletMatch[3]}`, { lineGap: 2 });
        continue;
      }
      ...
      if (olMatch) {
        doc.fontSize(10).font("Helvetica").fillColor(C.text)
          .text(`${olMatch[1]}.`, ML + 16, doc.y, { continued: true, width: 16 })
      ...
      if (hasFormatting) {
        ...
        if (segments.length > 0) {
          ...
          for (let si = 0; si < segments.length; si++) {
            const s = segments[si];
            const fontName = s.mono ? "Courier" : s.bold ? "Helvetica-Bold" : s.italic ? "Helvetica-Oblique" : "Helvetica";
            const color = s.mono ? "#DC2626" : C.text;
            if (s.mono) {
              // Inline code: draw background
              const textW = doc.font(fontName).fontSize(9).widthOfString(s.text);
              doc.rect(doc.x, doc.y - 1, textW + 6, 14).fill(C.codeBg);
              doc.font(fontName).fontSize(9).fillColor(color).text(s.text, doc.x + 3, doc.y, {
                continued: si < segments.length - 1, lineGap: 2,
              });
            } else {
              doc.font(fontName).fontSize(10).fillColor(color).text(s.text, undefined, undefined, {
      ...
      // ── Regular paragraph ──────────────────────────────────────
      doc.fontSize(10).font("Helvetica").fillColor(C.text)
        .text(line, ML, doc.y, { width: CW, lineGap: 3, align: "justify" });
    }
    ...
    // Go back and add headers/footers to pages 3+ (content pages, skip cover and TOC)
    // Page 0 = cover, Page 1 = TOC, Pages 2+ = content
    for (let i = 2; i < totalPages; i++) {
      ...
      doc.moveTo(ML, PH - 52).lineTo(PW - MR, PH - 52).strokeColor(C.border).lineWidth(0.5).stroke();
      doc.text(`${title}`, ML, PH - 48, { width: CW * 0.6, align: "left" });
      doc.text(`Page ${i + 1} of ${totalPages}`, ML, PH - 48, { width: CW, align: "right" });
      doc.restore();
    }

    // Add header/footer to TOC page
    doc.switchToPage(1);
    doc.save();
    doc.fontSize(7).font("Helvetica").fillColor(C.muted);
    doc.moveTo(ML, 54).lineTo(PW - MR, 54).strokeColor(C.border).lineWidth(0.3).stroke();
    ...
    doc.fontSize(8).font("Helvetica").fillColor(C.muted);
    doc.moveTo(ML, PH - 52).lineTo(PW - MR, PH - 52).strokeColor(C.border).lineWidth(0.5).stroke();
    doc.text("Table of Contents", ML, PH - 48, { width: CW * 0.6, align: "left" });
    doc.text(`Page 2 of ${totalPages}`, ML, PH - 48, { width: CW, align: "right" });
    doc.restore();
    ...
    return {
      ...
      message: `PDF report "${title}" created successfully (${totalPages} pages, with cover, TOC, and page numbers). Download available.`,
    };
  }),
});
...
export const createDocxDocumentTool = tool({
  ...
  execute: safeJson(async ({ title, content, filename, author }) => {
    ...
    const coverChildren: unknown[] = [
      ...
      new Paragraph({
        children: [
          new TextRun({ text: title, bold: true, size: 56, font: "Calibri", color: "1E3A5F" }),
        ],
      ...
      new Paragraph({
        children: [
          new TextRun({ text: `Date: ${dateStr}`, size: 22, color: "6B7280", font: "Calibri" }),
        ],
        alignment: AlignmentType.CENTER,
        spacing: { after: 200 },
      }),
    ...
    for (const line of lines) {
      ...
      if (line.trim().startsWith("```")) {
        if (inCodeBlock) {
          // End code block — render with language label
          const codeText = codeLines.join("\n");
          const label = codeLang ? `${codeLang.toUpperCase()} ` : "";
          contentChildren.push(
            new Paragraph({
              children: [
                new TextRun({ text: `${label}Code`, bold: true, size: 16, font: "Calibri", color: "2563EB" }),
              ],
              spacing: { before: 160, after: 40 },
              shading: { fill: "F1F5F9" },
            }),
          );
          // Add each line as a separate paragraph for proper formatting
          const codeParagraphs = codeText.split("\n");
          for (const cl of codeParagraphs) {
            contentChildren.push(
              new Paragraph({
                children: [
                  new TextRun({ text: cl || " ", size: 17, font: "Courier New", color: "334155" }),
                ],
                shading: { fill: "F8FAFC" },
      ...
      if (line.trim().startsWith("> ")) {
        ...
        if (blockquoteLines.length > 0) {
          ...
          contentChildren.push(
            new Paragraph({
              children: [
                new TextRun({ text: bqText, italics: true, size: 22, font: "Calibri", color: "1E3A5F" }),
              ],
        ...
        blockquoteLines = [];
        // Don't continue — let the line be processed normally below
      }
      ...
      if (line.trim().startsWith("|")) {
        ...
        if (tableRows.length > 0) {
          ...
          const colW = Math.floor(9000 / colCount); // Using DXA (twips)
            ...
            new TableRow({
              children: row.map((cell) =>
                new TableCell({
                  ...
                  shading: rowIdx === 0 ? { fill: "1E3A5F" } : rowIdx % 2 === 0 ? { fill: "F8FAFC" } : undefined,
                }),
              ),
            }),
          );
          try {
            contentChildren.push(
              new Table({ rows, width: { size: 9000, type: WidthType.DXA } }) as never,
            );
      ...
      if (h1Match) {
        contentChildren.push(
          new Paragraph({
            children: [new TextRun({ text: h1Match[1], bold: true, size: 36, font: "Calibri", color: "1E3A5F" })],
      ...
      if (h2Match) {
        contentChildren.push(
          new Paragraph({
            children: [new TextRun({ text: h2Match[1], bold: true, size: 28, font: "Calibri", color: "2563EB" })],
    ...
    if (inBlockquote && blockquoteLines.length > 0) {
      contentChildren.push(
        new Paragraph({
          children: [
            new TextRun({ text: blockquoteLines.join(" "), italics: true, size: 22, font: "Calibri", color: "1E3A5F" }),
          ],
          ...
          headers: {
            default: new Header({
              children: [
                new Paragraph({
                  children: [
                    new TextRun({ text: title, size: 16, color: "9CA3AF", font: "Calibri", italics: true }),
                  ],
...
// ---------------------------------------------------------------------------
// Create PPTX Presentation Tool (using pptxgenjs)
// ---------------------------------------------------------------------------

export const createPptxPresentationTool = tool({
  description: "Create a professional PowerPoint (.pptx) presentation and return it as a downloadable file. Supports multiple slides with titles, bullet points, notes, and various layouts. Use this when the user asks to create a presentation, slideshow, pitch deck, or any PPTX file.",
  inputSchema: zodSchema(z.object({
    title: z.string().describe("Presentation title"),
    slides: z.array(z.object({
      layout: z.enum(["title", "content", "two_content", "blank", "section"]).optional().describe("Slide layout (default: 'content')"),
      title_text: z.string().optional().describe("Slide title"),
      body_items: z.array(z.string()).optional().describe("Bullet points for the slide body"),
      notes: z.string().optional().describe("Speaker notes for this slide"),
    })).min(1).describe("Array of slides. Each slide can have a title, bullet points, and speaker notes."),
    filename: z.string().optional().describe("Output filename (without extension). Default: derived from title"),
  })),
  execute: safeJson(async ({ title, slides, filename }) => {
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      if (layout === "title" || layout === "section") {
        ...
        pptSlide.addText(slide.title_text || title, {
          x: 0.5, y: 1.5, w: "90%", h: 1.5,
          fontSize: 32, fontFace: "Arial", color: "111827", bold: true,
          align: "center", valign: "middle",
        });
        if (slide.body_items && slide.body_items.length > 0) {
          pptSlide.addText(slide.body_items.join("\n"), {
            x: 1, y: 3.2, w: "80%", h: 1.5,
            fontSize: 16, fontFace: "Arial", color: "6B7280",
        ...
        // Two-column layout
        if (slide.title_text) {
          pptSlide.addText(slide.title_text, {
            x: 0.5, y: 0.3, w: "90%", h: 0.8,
            fontSize: 24, fontFace: "Arial", color: "111827", bold: true,
          });
        }
        const items = slide.body_items || [];
        const mid = Math.ceil(items.length / 2);
        const leftItems = items.slice(0, mid);
        const rightItems = items.slice(mid);
        if (leftItems.length > 0) {
          pptSlide.addText(leftItems.map((t) => ({ text: t, options: { bullet: true, fontSize: 14, color: "374151" } })), {
            x: 0.5, y: 1.3, w: "45%", h: 4.5,
            fontFace: "Arial", valign: "top",
          });
        }
        if (rightItems.length > 0) {
          pptSlide.addText(rightItems.map((t) => ({ text: t, options: { bullet: true, fontSize: 14, color: "374151" } })), {
            x: "50%", y: 1.3, w: "45%", h: 4.5,
            fontFace: "Arial", valign: "top",
          });
        }
      } else {
        ...
        if (slide.body_items && slide.body_items.length > 0) {
          pptSlide.addText(slide.body_items.map((t) => ({ text: t, options: { bullet: true, fontSize: 16, color: "374151", paraSpaceAfter: 8 } })), {
            x: 0.7, y: 1.4, w: "85%", h: 4.2,
    ...
    return {
      ...
      message: `PowerPoint presentation "${title}" created successfully with ${slides.length} slide(s). Download available.`,
    };
  }),
});
