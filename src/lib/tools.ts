...
export const createPdfReportTool = tool({
  ...
  execute: safeJson(
    async ({
      ...
      const addFooter = () => {
        ...
        doc.moveTo(ML, PH - 52)
          ...
          .lineWidth(0.5)
          .stroke();
        doc.text(`Page ${pn}`, ML, PH - 48, { width: CW, align: "right" });
        doc.text(
          `${title}`,
          ML,
          PH - 48,
          { width: CW * 0.6, align: "left" },
        );
        doc.restore();
      };
      ...
      doc.text("Generated by Klawhub Agent Hub", 0, PH - 18, {
        width: PW,
        align: "center",
      });
      ...
      for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
        ...
        if (/^---+$/.test(line.trim())) {
          ensureSpace(40);
          doc.moveDown(0.4);
          // Decorative section-break divider
          const sby = doc.y;
          doc
            ...
            for (let si = 0; si < segments.length; si++) {
              ...
              if (s.mono) {
                ...
                const textW = doc
                  .font(fontName)
                  .fontSize(9)
                  .widthOfString(s.text);
                doc
                  .rect(doc.x, doc.y - 1, textW + 6, 14)
                  .fill(C.codeBg);
                doc
                  .font(fontName)
                  .fontSize(9)
                  .fillColor("#DC2626")
                  .text(s.text, doc.x + 3, doc.y, {
                    continued: !isLast,
                    lineGap: 2,
                  });
              } else if (s.link) {
                ...
                doc
                  ...
                  .fontSize(10)
                  .fillColor("#2563EB")
                  .text(s.text, undefined, undefined, {
                    continued: !isLast,
                    lineGap: 3,
                    link: s.link,
                    underline: true,
    ...
    .lineWidth(0.5)
    .stroke();
      doc.text("Table of Contents", ML, PH - 48, {
        width: CW * 0.6,
        align: "left",
      });
      doc.text(`Page 2 of ${totalPages}`, ML, PH - 48, {
        width: CW,
        align: "right",
      });
      doc.restore();
      ...
      .stroke();
        ...
        doc.text(`Page ${i + 1} of ${totalPages}`, ML, PH - 48, {
          width: CW,
          align: "right",
        });
        doc.restore();
      ...
      if (watermark && watermark.text) {
        const wmOpacity = Math.min(1, Math.max(0.01, watermark.opacity ?? 0.08));
        const wmAngle = ((watermark.rotation ?? 45) * Math.PI) / 180;
        const wmSize = watermark.fontSize ?? 52;
        ...
        for (let i = 1; i < totalPages; i++) {
          ...
          doc
            ...
            .font("Helvetica-Bold")
            .fillColor("#9CA3AF");
          doc.text(watermark.text, -250, -wmSize / 2, {
            width: 500,
            align: "center",
          });
          doc.restore();
