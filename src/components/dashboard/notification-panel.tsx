...
function NotifRow({ notif }: { notif: AppNotification }) {
  ...
  return (
    ...
    >
      {/* Unread dot */}
      {!notif.read && (
        <span className="absolute left-1.5 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_6px_rgba(16,185,129,0.4)]" />
      )}
        ...
        <div className="flex items-start justify-between gap-2">
          <p
            className={cn(
              ...
              notif.read ? "text-slate-400 font-normal" : "text-slate-200 font-medium",
            )}
          >
            {notif.title}
          </p>
      ...
      <button
        ...
        className="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-md hover:bg-red-500/10 text-slate-500 hover:text-red-400"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed right-0 top-0 z-[70] h-full w-full sm:w-[420px] bg-[#0d1117] border-l flex flex-col"
            style={{ borderColor: "rgba(255,255,255,0.06)", boxShadow: "-8px 0 32px rgba(0,0,0,0.4)" }}
          >
            {/* ---- Header ---- */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06]">
              <div className="flex items-center gap-2.5">
                ...
                {unreadCount > 0 && (
                  <span className="inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 text-[11px] font-bold rounded-full bg-emerald-500 text-white">
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="p-2 rounded-lg hover:bg-white/[0.04] text-slate-400 hover:text-white transition-colors disabled:opacity-50"
                  title="Refresh"
                >
                  <Loader2
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
              {isLoading && notifications.length === 0 ? (
                ...
                <div>
                  ...
                  {recent.length > 0 && (
                    <div>
                      <div className="px-5 py-2.5 text-[11px] font-semibold uppercase tracking-wider text-slate-600">
                        Recent
                      </div>
...
export function NotificationBell() {
  ...
  return (
    <button
      onClick={togglePanel}
      className="relative w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 text-slate-400 hover:text-white hover:bg-white/[0.04]"
    >
