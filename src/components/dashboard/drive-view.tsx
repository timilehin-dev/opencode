        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-semibold text-foreground">
              Files & Folders <span className="ml-2 text-sm font-normal text-muted-foreground">({drvFiles.length})</span>
            </h2>
            <button
              onClick={() => setDrvTab("create")}
              className="inline-flex items-center gap-1.5 bg-green-600 hover:bg-green-500 text-foreground px-3 py-1.5 rounded-lg text-sm font-medium transition-colors"
            >
              <PlusIcon /> New
            </button>
          </div>
                  ...
                  return (
                    <div key={file.id} className="grid grid-cols-[1fr_auto_auto] px-5 py-2.5 hover:bg-[#1e293b] transition-colors border-b border-slate-700/30 last:border-b-0 items-center">
                      <div className="flex items-center gap-2.5 text-sm min-w-0">
                        {isFolder ? <FolderIcon className="text-green-400" /> : <FileIcon className="text-muted-foreground" />}
                        {file.webViewLink ? (
                          <a href={file.webViewLink} target="_blank" rel="noopener noreferrer"
                            className="text-foreground font-medium hover:text-green-400 transition-colors truncate">
                            {file.name}
                          </a>
                        ) : (
      ...
      {drvTab === "create" && (
        <div className="max-w-2xl">
          <div className="bg-card border border-border rounded-xl p-5">
            <h2 className="text-lg font-semibold text-foreground mb-4">Create Folder</h2>
            {folderSuccess && (
            ...
            <div className="space-y-3">
              <div>
                ...
                <input type="text" placeholder="My New Folder" value={newFolderName} onChange={(e) => setNewFolderName(e.target.value)}
                  className="w-full bg-[#0f172a] border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-foreground placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-green-500/50 focus:border-green-500" />
              </div>
              <button onClick={handleCreateFolder} disabled={creatingFolder || !newFolderName.trim()}
                className="inline-flex items-center gap-2 bg-green-600 hover:bg-green-500 disabled:bg-muted disabled:text-muted-foreground text-foreground px-5 py-2.5 rounded-lg text-sm font-medium transition-colors">
