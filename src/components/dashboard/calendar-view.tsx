        ...
        <div>
          ...
          {calEvents.length === 0 ? (
            <div className="bg-card border border-border rounded-xl p-12 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">Your calendar events will appear here.</p>
            </div>
          ) : (
              ...
              {groupedEvents.map(({ dateKey, label, events }) => (
                <div key={dateKey}>
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    <h3 className="text-sm font-semibold text-foreground whitespace-nowrap">{label}</h3>
                  ...
                  <div className="space-y-2">
                    {events.map((evt) => {
                      ...
                      return (
                        <div
                          key={evt.id}
                          className="bg-card border border-border rounded-xl px-4 py-3.5 hover:bg-[#1e293b] hover:border-border transition-all duration-200 group"
                        >
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex flex-col items-center gap-1.5 pt-0.5 flex-shrink-0 w-14">
                              ...
                              {timeRange && (
                                <span className="text-xs text-muted-foreground leading-tight text-center">
                                  {isAllDay ? (
                                    <span className="font-medium text-muted-foreground">All day</span>
                                  ) : (
                                    <>
                                      <span className="block">{formatTime(evt.start.dateTime)}</span>
                                      {formatTime(evt.end.dateTime) && (
                                        <span className="block text-muted-foreground">{formatTime(evt.end.dateTime)}</span>
                                      )}
                                    </>
                                  )}
                                </span>
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              {/* Description preview */}
                              {evt.description && (
                                <p className="text-xs text-muted-foreground mt-1.5 truncate max-w-md">
                                  {evt.description}
                                </p>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-1 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                              {evt.htmlLink && (
                                <a
                                  ...
                                  className="text-muted-foreground hover:text-blue-600 transition-colors p-1.5 rounded-lg hover:bg-blue-500/10"
                              ...
                              <button
                                ...
                                className="text-muted-foreground hover:text-red-400 transition-colors p-1.5 rounded-lg hover:bg-red-500/10 disabled:opacity-50"
                                title="Delete event"
                              >
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#0f172a] border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-foreground placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-colors"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Start Date & Time</label>
                  <input
                    type="datetime-local"
                    value={evtStart}
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-[#0f172a] border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-colors"
                  >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-[#0f172a] border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-foreground placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 resize-y transition-colors"
                />
              </div>

              {/* Attendees */}
              <div>
                <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Attendees (comma-separated)</label>
                <input
                  type="text"
              ...
              <div className="flex items-center gap-3 py-1">
                <label className="relative inline-flex items-center cursor-pointer">
                  ...
                  />
                  <div className="w-9 h-5 bg-muted peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-500/50 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-card after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600" />
                </label>
                <span className="text-sm text-foreground">Add Google Meet video conferencing</span>
              </div>
              ...
              <button
                ...
                className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:bg-muted disabled:text-muted-foreground text-foreground px-5 py-2.5 rounded-lg text-sm font-medium transition-colors"
              >
                {creatingEvent && (
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-foreground">
              Your Calendars
              <span className="ml-2 text-sm font-normal text-muted-foreground">{calendars.length}</span>
            </h2>
          </div>
            ...
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              {calendars.map((cal) => (
                ...
                >
                  ...
                  <span className="text-sm text-foreground font-medium flex-1 truncate">{cal.summary}</span>
                  {cal.primary && (
