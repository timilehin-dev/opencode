...
- "type": one of "recover_stale", "create_agent_task", "send_a2a_message", "send_notification", "initiate_collaboration", "initiate_escalation", "skip"
- "finding": which finding this addresses
- "agent": target agent ID (for create_agent_task, send_a2a_message, initiate_collaboration)
...
- "collaboration_reason": why agents should collaborate (for initiate_collaboration)
- "collaboration_context": what context to share (for initiate_collaboration)
- "escalation_severity": "low", "medium", "high", or "critical" (for initiate_escalation)
- "escalation_impact": what is affected (for initiate_escalation)
...
- When one agent's failure impacts another's work, use initiate_collaboration to connect them
- When an issue is critical and needs immediate human attention, use initiate_escalation
...
async function executeAction(action, db) {
  switch (action.type) {
    ...
    case 'initiate_collaboration':
      return executeInitiateCollaboration(db, action);
    case 'initiate_escalation':
      return executeInitiateEscalation(db, action);
    case 'skip':
...
// --- Action: Initiate collaboration between agents (Phase 5) ---

async function executeInitiateCollaboration(db, action) {
  const { agent, collaboration_reason, collaboration_context, finding, reason } = action;

  if (!agent || !collaboration_reason) {
    console.warn('[act] Missing agent or collaboration_reason in initiate_collaboration action');
    return null;
  }
  ...
  try {
    ...
    const initResult = await db.query(
      `INSERT INTO a2a_initiations (initiator_agent, target_agent, initiation_type, subject, context, urgency, status)
      ...
      [
        ...
        `Chief Engineer: ${finding || 'Collaboration Opportunity'}`,
        JSON.stringify({ reason: collaboration_reason, context: collaboration_context, source: 'autonomous-engineer', finding }),
        'normal',
      ]
    );

    const initiationId = initResult.rows[0].id;

    // Send A2A message to the target agent
    const msgResult = await db.query(
      `INSERT INTO a2a_messages (from_agent, to_agent, type, topic, payload, priority, status)
       VALUES ($1, $2, 'collaboration', $3, $4, 'normal', 'delivered')
       RETURNING id`,
      [
        CONFIG.engineer.id,
        agent,
        `[COLLABORATION OPPORTUNITY] ${collaboration_reason.slice(0, 80)}`,
        JSON.stringify({
          source: 'autonomous-engineer',
          finding: finding,
          content: `The Autonomous Engineer has identified a collaboration opportunity:\n\n**Reason:** ${collaboration_reason}\n\n**Context:** ${collaboration_context || 'See system metrics in your inbox.'}\n\n**Finding:** ${finding || 'System assessment'}\n\nPlease check your inbox and consider reaching out to relevant team members using initiate_contact or offer_assistance.`,
          shift: state.timestamp,
        }),
      ]
    );

    state.actions.push(`Initiated collaboration with ${agent}: "${collaboration_reason.slice(0, 80)}"`);
    state.decisions.push({
      finding: finding,
      decision: 'initiate_collaboration',
      target_agent: agent,
      initiation_id: initiationId,
      reason: reason || collaboration_reason,
    });

    console.log(`[act] Initiated collaboration #${initiationId} with ${agent}`);
    return initiationId;
  } catch (err) {
    console.error(`[act] Failed to initiate collaboration with ${agent}:`, err.message);
    state.errors.push(`Collaboration initiation failed (${agent}): ${err.message}`);
    return null;
  }
}

// --- Action: Escalate an issue (Phase 5) ---

async function executeInitiateEscalation(db, action) {
  const { escalation_severity: severity, escalation_impact: impact, finding, reason } = action;

  if (!severity || !impact) {
    console.warn('[act] Missing escalation_severity or escalation_impact in initiate_escalation action');
    return null;
  }

  try {
    const urgency = severity === 'critical' ? 'critical' : severity === 'high' ? 'high' : 'normal';
    const shouldNotifyUser = severity === 'critical' || severity === 'high';

    // Log the escalation
    const initResult = await db.query(
      `INSERT INTO a2a_initiations (initiator_agent, target_agent, initiation_type, subject, context, urgency, status)
       VALUES ($1, 'general', 'escalation', $2, $3, $4, 'delivered')
       RETURNING id`,
      [
        CONFIG.engineer.id,
        `[ESCALATION - ${severity.toUpperCase()}] ${finding || 'System Issue'}`,
        JSON.stringify({ finding, impact, severity, reason, source: 'autonomous-engineer', metrics: { pending: state.metrics.pending_tasks, failed_24h: state.metrics.recent_failures } }),
        urgency,
      ]
    );
    ...
    // Send A2A message to general
    await db.query(
      `INSERT INTO a2a_messages (from_agent, to_agent, type, topic, payload, priority, status)
       VALUES ($1, 'general', 'request', $2, $3, $4, 'delivered')
       RETURNING id`,
      [
        CONFIG.engineer.id,
        `[ESCALATION - ${severity.toUpperCase()}] ${finding || 'System Issue'}`,
        JSON.stringify({
          source: 'autonomous-engineer',
          content: `The Autonomous Engineer is escalating an issue:\n\n**Severity:** ${severity.toUpperCase()}\n**Issue:** ${finding}\n**Impact:** ${impact}\n**Reason:** ${reason || 'See system metrics.'}\n\nPlease assess and take appropriate action.${shouldNotifyUser ? '\n\n**User notification triggered.**' : ''}`,
          shift: state.timestamp,
        }),
    ...
    // Notify user if critical/high
    if (shouldNotifyUser) {
      try {
        await db.query(
          `INSERT INTO proactive_notifications (type, title, message, severity, source_agent, metadata)
           VALUES ($1, $2, $3, $4, $5, $6)`,
          [
            'escalation',
            `System Escalation: ${finding || 'Issue detected'}`,
            `**Severity:** ${severity.toUpperCase()}\n**Issue:** ${finding}\n**Impact:** ${impact}\n**Reason:** ${reason || 'Automated detection by Autonomous Engineer.'}`,
            severity,
            CONFIG.engineer.id,
            JSON.stringify({ source: 'autonomous-engineer', initiation_id: initiationId }),
          ]
        );
      } catch { /* notification non-critical */ }
    }

    state.actions.push(`Escalated issue to General (severity: ${severity}): "${finding}"`);
    state.decisions.push({
      finding: finding,
    ...
    console.log(`[act] Escalated issue #${initiationId} (severity: ${severity})`);
    return initiationId;
  } catch (err) {
    console.error('[act] Failed to escalate issue:', err.message);
    state.errors.push(`Escalation failed: ${err.message}`);
    return null;
  }
}
