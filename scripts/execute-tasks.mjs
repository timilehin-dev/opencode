#!/usr/bin/env node
// ---------------------------------------------------------------------------
// Klawhub Task Executor — Standalone script for GitHub Actions
//
...
- **Klawhub General** — Chief Orchestrator with ALL tools (Gmail, Calendar, Drive, Sheets, Docs, GitHub, Vercel, Web, Vision, Image Gen, Design, Data Analysis). Handles complex multi-domain tasks.
...
## Who You Are
You are the research analyst of the Klawhub Agent Hub — modeled after a senior analyst at a top research firm. You specialize in deep multi-source research, cross-referencing, synthesis, and producing professional research briefs.
...
async function readWebPage(url) {
  const res = await fetch(url, {
    headers: { "User-Agent": "Mozilla/5.0 (compatible; KlawhubBot/1.0)" },
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.08);overflow:hidden;max-width:600px;width:100%;">
          ...
          <tr>
            <td style="padding:16px 32px;border-top:1px solid #e8e8ee;text-align:center;">
              <p style="margin:0;font-size:12px;color:#999999;">This email was sent by your Klawhub Mail Agent.</p>
            </td>
          </tr>
        </table>
      </td>
...
function buildToolMap(agentId) {
  ...
  return {
    ...
    initiate_contact: tool({
      description: `PROACTIVELY reach out to another agent to share information, propose collaboration, or coordinate work. Use when: you've completed work another agent needs, you want to propose collaboration, you've discovered something relevant to another agent. For task delegation use schedule_agent_task or query_agent instead.`,
      inputSchema: zodSchema(z.object({
        to_agent: z.enum(["general", "mail", "code", "data", "creative", "research", "ops"]).describe("Target agent"),
        subject: z.string().describe("Short subject line"),
        message: z.string().describe("Full message with all relevant context"),
        urgency: z.enum(["low", "normal", "high", "critical"]).optional().describe("Urgency (default: normal)"),
        propose_collaboration: z.boolean().optional().describe("Frame as collaboration proposal"),
        related_task_id: z.number().optional().describe("Related task ID"),
      })),
      execute: safeJsonWrap(async ({ to_agent, subject, message, urgency, propose_collaboration, related_task_id }) => {
        ...
        try {
          ...
          await pool.query(
            ...
            [fromAgent, to_agent, "contact", subject, JSON.stringify({ message: message.slice(0, 1000), propose_collaboration }), urgency || "normal", related_task_id ?? null]
          );
          // Send A2A message
          const collabNote = propose_collaboration ? "\n\n**This is a collaboration proposal — please respond if interested.**" : "";
          const msgRes = await pool.query(
            `INSERT INTO a2a_messages (from_agent, to_agent, type, topic, payload, priority) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`,
            [fromAgent, to_agent, propose_collaboration ? "collaboration" : "context_share",
             `${propose_collaboration ? "[Collaboration Proposal] " : ""}${subject}`,
             JSON.stringify({ content: `${fromAgent} is reaching out:\n\n**Subject**: ${subject}\n\n${message}${collabNote}`, source: "initiate_contact" }),
             urgency === "critical" ? "urgent" : urgency || "normal"]
          );
          return { success: true, messageId: msgRes.rows[0]?.id, to: to_agent, subject, urgency: urgency || "normal" };
        } catch (err) {
    ...
    request_help: tool({
      description: `Request HELP from another specialist agent when stuck, need expertise, or encounter problems. Structured: explain the problem, what you tried, what you need. The target agent receives full context and can advise, take over, or collaborate.`,
      inputSchema: zodSchema(z.object({
        from_agent: z.enum(["general", "mail", "code", "data", "creative", "research", "ops"]).describe("Your agent ID"),
        to_agent: z.enum(["general", "mail", "code", "data", "creative", "research", "ops"]).describe("Agent to ask for help"),
        problem: z.string().describe("Clear description of the problem"),
        what_tried: z.string().describe("What you've already tried"),
        what_needed: z.string().describe("Specifically what help you need"),
        urgency: z.enum(["low", "normal", "high", "critical"]).optional().describe("Urgency (default: high)"),
        related_task_id: z.number().optional().describe("Related task ID"),
        context_data: z.string().optional().describe("Additional context (error logs, code snippets)"),
      })),
      execute: safeJsonWrap(async ({ from_agent, to_agent, problem, what_tried, what_needed, urgency, related_task_id, context_data }) => {
        const effectiveFrom = from_agent || currentAgentId;
        const effectiveUrgency = urgency || "high";
        try {
          await pool.query(
            `INSERT INTO a2a_initiations (initiator_agent, target_agent, initiation_type, subject, context, urgency, status, related_task_id)
             VALUES ($1, $2, $3, $4, $5, $6, 'delivered', $7)`,
            [effectiveFrom, to_agent, "help_request", `Help request: ${problem.slice(0, 100)}`,
             JSON.stringify({ problem, what_tried, what_needed, context_data: context_data?.slice(0, 2000) }),
             effectiveUrgency, related_task_id ?? null]
          );
          const contextSection = context_data ? `\n\n**Additional Context:**\n\`\`\`\n${context_data.slice(0, 2000)}\n\`\`\`` : "";
          const msgRes = await pool.query(
            `INSERT INTO a2a_messages (from_agent, to_agent, type, topic, payload, priority) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`,
            [effectiveFrom, to_agent, "request",
             `[HELP REQUEST] ${problem.slice(0, 80)}`,
             JSON.stringify({ content: `${effectiveFrom} is requesting your help:\n\n**Problem:** ${problem}\n\n**What I've Already Tried:** ${what_tried}\n\n**What I Need:** ${what_needed}\n\n**Urgency:** ${effectiveUrgency.toUpperCase()}${contextSection}`, source: "request_help" }),
             effectiveUrgency === "critical" ? "urgent" : effectiveUrgency]
          );
          return { success: true, messageId: msgRes.rows[0]?.id, from: effectiveFrom, to: to_agent, urgency: effectiveUrgency };
        } catch (err) {
          return { success: false, error: err.message };
        }
      }),
    ...
    offer_assistance: tool({
      description: `PROACTIVELY OFFER help to another agent based on observations or knowledge. Use when you notice another agent struggling, have expertise relevant to their task, or completed work that helps them.`,
      inputSchema: zodSchema(z.object({
        to_agent: z.enum(["general", "mail", "code", "data", "creative", "research", "ops"]).describe("Agent to offer help to"),
        what_i_can_help: z.string().describe("Specifically what you can help with"),
        why_relevant: z.string().describe("Why this help is relevant"),
        my_expertise: z.string().optional().describe("Your relevant expertise or tools"),
        related_task_id: z.number().optional().describe("Related task ID"),
      })),
      execute: safeJsonWrap(async ({ to_agent, what_i_can_help, why_relevant, my_expertise, related_task_id }) => {
        const fromAgent = currentAgentId;
        try {
          await pool.query(
            `INSERT INTO a2a_initiations (initiator_agent, target_agent, initiation_type, subject, context, urgency, status, related_task_id)
             VALUES ($1, $2, $3, $4, $5, 'normal', 'delivered', $6)`,
            [fromAgent, to_agent, "assistance_offer", `Assistance offer: ${what_i_can_help.slice(0, 100)}`,
             JSON.stringify({ what_i_can_help, why_relevant, my_expertise }), related_task_id ?? null]
          );
          const expertiseSection = my_expertise ? `\n**My Relevant Expertise:** ${my_expertise}` : "";
          const msgRes = await pool.query(
            `INSERT INTO a2a_messages (from_agent, to_agent, type, topic, payload, priority) VALUES ($1, $2, $3, $4, $5, 'normal') RETURNING id`,
            [fromAgent, to_agent, "collaboration",
             `[ASSISTANCE OFFER] ${what_i_can_help.slice(0, 80)}`,
             JSON.stringify({ content: `${fromAgent} is offering assistance:\n\n**What I Can Help With:** ${what_i_can_help}\n\n**Why This Is Relevant:** ${why_relevant}${expertiseSection}`, source: "offer_assistance" })]
          );
          return { success: true, messageId: msgRes.rows[0]?.id, from: fromAgent, to: to_agent };
        } catch (err) {
          return { success: false, error: err.message };
        }
      }),
    ...
    observe_agent: tool({
      description: `OBSERVE what another agent has been doing recently. Returns current status, pending tasks, recent completed/failed tasks, A2A messages, and activity log. Use BEFORE initiating contact to make informed decisions.`,
      inputSchema: zodSchema(z.object({
        agent_id: z.enum(["general", "mail", "code", "data", "creative", "research", "ops"]).describe("Agent to observe"),
        ...
        hours_back: z.number().optional().describe("Hours back to look (default: 24)"),
      })),
      execute: safeJsonWrap(async ({ agent_id, include_tasks, include_messages, include_activity, hours_back }) => {
        const hours = hours_back || 24;
        const since = `NOW() - INTERVAL '${hours} hours'`;
        const result = { agent: agent_id, observedAt: new Date().toISOString(), hoursBack: hours };
        try {
          // Current status
          const statusRes = await pool.query(`SELECT agent_id, status, current_task, last_activity, tasks_completed, messages_processed FROM agent_status WHERE agent_id = $1`, [agent_id]);
          result.currentStatus = statusRes.rows[0] || { status: "unknown" };
          // Pending tasks
          const queueRes = await pool.query(`SELECT id, task, status, priority, trigger_type, created_at, attempts, max_attempts FROM agent_tasks WHERE agent_id = $1 AND status IN ('pending', 'running', 'queued') ORDER BY priority, created_at ASC`, [agent_id]);
          result.pendingTasks = queueRes.rows.map(r => ({ id: r.id, task: String(r.task || "").slice(0, 150), status: r.status, priority: r.priority, trigger: r.trigger_type }));
          // Recent tasks
          if (include_tasks !== false) {
            const tasksRes = await pool.query(`SELECT id, task, status, priority, result, error, completed_at, duration_ms FROM agent_tasks WHERE agent_id = $1 AND completed_at > ${since} ORDER BY completed_at DESC LIMIT 20`, [agent_id]);
            result.recentTasks = tasksRes.rows.map(r => ({ id: r.id, task: String(r.task || "").slice(0, 150), status: r.status, result: r.result ? String(r.result).slice(0, 200) : null, error: r.error ? String(r.error).slice(0, 200) : null, completedAt: r.completed_at, durationMs: r.duration_ms }));
          }
          // Recent messages
          if (include_messages !== false) {
            const msgRes = await pool.query(`SELECT id, from_agent, to_agent, type, topic, priority, is_read, created_at, payload->>'content' as content_preview FROM a2a_messages WHERE (from_agent = $1 OR to_agent = $1) AND created_at > ${since} ORDER BY created_at DESC LIMIT 20`, [agent_id]);
            result.recentMessages = msgRes.rows.map(r => ({ id: r.id, from: r.from_agent, to: r.to_agent, type: r.type, topic: r.topic, priority: r.priority, isRead: r.is_read, createdAt: r.created_at, preview: r.content_preview ? String(r.content_preview).slice(0, 150) : null }));
          }
          // Activity
          if (include_activity !== false) {
            const actRes = await pool.query(`SELECT action, details, tool_used, created_at FROM agent_activity WHERE agent_id = $1 AND created_at > ${since} ORDER BY created_at DESC LIMIT 15`, [agent_id]);
            result.recentActivity = actRes.rows.map(r => ({ action: r.action, tool: r.tool_used, details: r.details ? String(r.details).slice(0, 150) : null, createdAt: r.created_at }));
          }
          // Project tasks
          const projRes = await pool.query(`SELECT pt.id, p.name as project, pt.title, pt.status, pt.priority FROM project_tasks pt JOIN projects p ON p.id = pt.project_id WHERE pt.assigned_agent = $1 AND pt.status IN ('pending', 'in_progress', 'blocked') ORDER BY pt.priority LIMIT 10`, [agent_id]);
          result.activeProjectTasks = projRes.rows;
        } catch (err) {
          return { success: false, error: err.message };
        }
        return result;
    ...
    escalate_to_chief: tool({
      description: `ESCALATE an issue to the General (Chief) agent for decision-making, intervention, or user notification. Use when: a task failed multiple times, critical issue discovered, need decision above your authority, unresolvable blocker. Auto-notifies user for critical/high severity.`,
      inputSchema: zodSchema(z.object({
        issue: z.string().describe("Clear description of the issue"),
        severity: z.enum(["low", "medium", "high", "critical"]).describe("Issue severity"),
        impact: z.string().describe("Impact of this issue"),
        what_tried: z.string().optional().describe("What you've already tried"),
        recommended_action: z.string().optional().describe("What you think should be done"),
        notify_user: z.boolean().optional().describe("Also notify user (default: true for critical/high)"),
        related_task_id: z.number().optional().describe("Related task ID"),
      })),
      execute: safeJsonWrap(async ({ issue, severity, impact, what_tried, recommended_action, notify_user, related_task_id }) => {
        const fromAgent = currentAgentId;
        const shouldNotifyUser = notify_user !== undefined ? notify_user : (severity === "critical" || severity === "high");
        try {
          await pool.query(
            `INSERT INTO a2a_initiations (initiator_agent, target_agent, initiation_type, subject, context, urgency, status, related_task_id)
             VALUES ($1, 'general', 'escalation', $2, $3, $4, 'delivered', $5)`,
            [fromAgent, `[ESCALATION - ${severity.toUpperCase()}] ${issue.slice(0, 100)}`,
             JSON.stringify({ issue, impact, what_tried, recommended_action, severity }),
          ...
          const recSection = recommended_action ? `\n**Recommended Action:** ${recommended_action}` : "";
          const msgRes = await pool.query(
            `INSERT INTO a2a_messages (from_agent, to_agent, type, topic, payload, priority) VALUES ($1, 'general', 'request', $2, $3, $4) RETURNING id`,
            [fromAgent,
             `[ESCALATION - ${severity.toUpperCase()}] ${issue.slice(0, 80)}`,
             JSON.stringify({ content: `${fromAgent} is escalating:\n\n**Issue:** ${issue}\n\n**Severity:** ${severity.toUpperCase()}\n\n**Impact:** ${impact}${triedSection}${recSection}\n\nPlease assess and take action.${shouldNotifyUser ? "\n\n**User notification requested.**" : ""}`, source: "escalate_to_chief" }),
             severity === "critical" ? "urgent" : severity === "high" ? "high" : "normal"]
          );
          if (shouldNotifyUser) {
            await pool.query(
              `INSERT INTO proactive_notifications (type, title, message, severity, source_agent, related_task_id) VALUES ($1, $2, $3, $4, $5, $6)`,
              ["escalation", `Escalation from ${fromAgent}: ${issue.slice(0, 80)}`, `**Agent:** ${fromAgent}\n**Severity:** ${severity.toUpperCase()}\n**Issue:** ${issue}\n**Impact:** ${impact}`, severity, fromAgent, related_task_id ?? null]
            ).catch(() => {});
          }
          return { success: true, messageId: msgRes.rows[0]?.id, severity, from: fromAgent, userNotified: shouldNotifyUser };
        } catch (err) {
          return { success: false, error: err.message };
        }
      }),
  ...
  try {
    ...
    try {
      ...
      for (const row of escalateResult.rows) {
        ...
        await sendNotification({
          agentId: "general",
          agentName: "Klawhub General",
          type: "alert",
          title: `Task auto-escalated after ${row.attempts} failures`,
          body: `Task #${row.id} ("${row.task.slice(0, 80)}...") has been escalated to Klawhub General for review.`,
          priority: "medium",
